...
# On these instances, a netplan override stub is written so that networkd
# can bring up the NIC once the driver is ready, allowing the cloud-init
# network stage to succeed without requiring a reboot.
NETPLAN_OVERRIDE_PRODUCT_ALLOW_LIST = [
    "hpc7a.96xlarge",
]
NETPLAN_OVERRIDE_PATH = "/etc/netplan/90-cloud-init-nic-override.yaml"
NETPLAN_OVERRIDE_DRIVER = "ena"
    ...
    def _get_data(self):
        ...
        if self.perform_dhcp_setup:  # Setup networking in init-local stage.
            ...
            if len(candidate_nics) < 1:
                product_name = (
                    dmi.read_dmi_data("system-product-name") or ""
                )
                if product_name in NETPLAN_OVERRIDE_PRODUCT_ALLOW_LIST:
                    LOG.warning(
                        "No NICs found on %s. Writing netplan override "
                        "stub to %s so networkd can bring up the NIC "
                        "when the %s driver is ready.",
                        product_name,
                        NETPLAN_OVERRIDE_PATH,
                        NETPLAN_OVERRIDE_DRIVER,
                    )
                    _write_netplan_override_stub(
                        NETPLAN_OVERRIDE_PATH,
                        NETPLAN_OVERRIDE_DRIVER,
                    )
                    return False
...
def _write_netplan_override_stub(path: str, driver: str) -> None:
    """Write a minimal netplan config stub that matches NICs by driver.

    This is used when no NICs are available during init-local on instance
    types with slow driver initialization. The stub enables networkd to
    bring up the interface once the driver is ready, allowing the
    cloud-init network stage to succeed.
    """
    content = (
        "# Generated by cloud-init to work around late NIC driver init.\n"
        "# This file will be overwritten by cloud-init on next boot.\n"
        "network:\n"
        "  version: 2\n"
        "  ethernets:\n"
        "    all-en:\n"
    ...
    util.write_file(path, content, mode=0o600)
