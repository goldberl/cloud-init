...
# are not ready when cloud-init-local runs. On these platforms, a netplan
# override stub is written so that networkd can bring up the NIC once the
# driver is ready, allowing the cloud-init network stage to succeed.
NETPLAN_OVERRIDE_BOARD_ALLOW_LIST = [
    "izumi",  # c3-metal
]
NETPLAN_OVERRIDE_PATH = "/etc/netplan/90-cloud-init-nic-override.yaml"
NETPLAN_OVERRIDE_DRIVER = "idpf"
    ...
    def _get_data(self):
        ...
        if self.perform_dhcp_setup:
            ...
            if not candidate_nics:
                board_name = (
                    dmi.read_dmi_data("baseboard-product-name") or ""
                )
                if board_name in NETPLAN_OVERRIDE_BOARD_ALLOW_LIST:
                    LOG.warning(
                        "No NICs found on board %s. Writing netplan "
                        "override stub to %s so networkd can bring up "
                        "the NIC when the %s driver is ready.",
                        board_name,
...
def _write_netplan_override_stub(path: str, driver: str) -> None:
    ...
    This is used when no NICs are available during init-local on platforms
    with slow driver initialization. The stub enables networkd to bring up
    the interface once the driver is ready, allowing the cloud-init network
    stage to succeed.
    """
    content = (
